...
class Swn:
    ...
    def compute_sentiment_scores_of_word(word, dictionary_indices):
        ...
        for r in rows:
            #print(str(word) + "(p,n) = (" + str(np_dictionary[r][1]) + "," + str(np_dictionary[r][2]) + ")")
            positive_score_sum += float(np_dictionary[r][1])
            negative_score_sum += float(np_dictionary[r][2])
        # print("\'" + str(word) + "\' p score: " + str(positive_score_sum) + "/" + str(len(rows)))
        # print("\'" + str(word) + "\' n score: " + str(negative_score_sum) + "/" + str(len(rows)))
    ...
    def sentiment_analysis(self, raw_tweets, cleaned_tweets):
        ...
        for c_tweet, r_tweet in zip(cleaned_tweets, raw_tweets):
            ...
            if not words:  # If tweet is empty (exception handling)
                ...
                for word in words:
                    ...
                    if len(dict_rows) == 0:  # If the word is not present in the dictionary.
                        ...
                        #print("\'" + str(word) + "\' is not in the lexicon. Assigning sentiment (0,0)")
                        ...
                        (positive_sentiment_score, negative_sentiment_score) = self.compute_sentiment_scores_of_word(
                            word, indices)
                        # print(
                        #     "Sentiment score of " + str(word) + " : " + "(" + str(positive_sentiment_score) + "," + str(
                ...
                # print("Calculation for tweet sentiment score = (" + str(positive_scores_sum) + " - " + str(
                #     negative_scores_sum) + ")/" + str(word_count))
                # print("Tweet sentiment score: " + str(tweet_sentiment_score))
            ...
            #print("==========Sentiment analysis using averages concluded==========")
